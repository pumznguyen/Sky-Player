...
import time
import random
from sky_music.backend import BackendHealth
...
class TelemetryLogger:
    ...
    def save(self) -> None:
        ...
        try:
            ...
            # 2. Compute precision timing stats
            latenesses = [r["lateness_us"] for r in self.records]
            send_durations = [r["send_duration_us"] for r in self.records]
            
            # Count threshold events
            over_2ms = sum(1 for lat in latenesses if lat > 2000)
            over_5ms = sum(1 for lat in latenesses if lat > 5000)
            over_10ms = sum(1 for lat in latenesses if lat > 10000)
            
            # Note hold times calculation
            hold_durations = []
            active_downs = {}
            for r in self.records:
                codes = [int(sc) for sc in r["scan_codes"].split(";") if sc]
            ...
            def calc_percentiles(values: list[int], track_thresholds: bool = False) -> dict[str, float]:
                if not values:
                    base = {"p50_us": 0.0, "p95_us": 0.0, "p99_us": 0.0, "max_us": 0.0, "avg_us": 0.0}
                    if track_thresholds:
                        base.update({"over_2ms": 0, "over_5ms": 0, "over_10ms": 0})
                    return base
            ...
            if self.backend_health is not None:
                backend_info = {
                    "panic_release_failures": self.backend_health.failed_release_count,
                    "failed_release_keys_final": [] # placeholders or detailed codes if desired
                }
            ...
            summary = {
                ...
                "total_events": len(self.records),
                "lateness_us": calc_percentiles(latenesses, track_thresholds=True),
                "send_duration_us": calc_percentiles(send_durations),
                "note_hold_duration_us": calc_percentiles(hold_durations),
                "backend": backend_info
            }
            
            # Save companion summary JSON
...
def inspect_telemetry_report(target_path: str) -> None:
